...
type Props = {
  ...
  targetId: string;
  /** Optional fallback seed (e.g. demo restaurant rating) */
  seedRating?: number;
  seedCount?: number;
};
...
export function ReviewsSection({ targetType, targetId, seedRating, seedCount }: Props) {
  ...
  const mineFn = useServerFn(myReview);
  const saveFn = useServerFn(upsertReview);
  const delFn = useServerFn(deleteReview);

  const key = ["reviews", targetType, targetId];
  const reviewsQ = useQuery({ queryKey: key, queryFn: () => listFn({ data: { target_type: targetType, target_id: targetId } }) });
  const sumQ = useQuery({ queryKey: [...key, "summary"], queryFn: () => sumFn({ data: { target_type: targetType, target_id: targetId } }) });
  const mineQ = useQuery({
    queryKey: [...key, "mine", user?.id ?? "none"],
    queryFn: () => mineFn({ data: { target_type: targetType, target_id: targetId } }),
    enabled: !!user,
  });
  ...
  useEffect(() => {
    if (mineQ.data) {
      setRating(mineQ.data.rating);
      setTitle(mineQ.data.title ?? "");
      setBody(mineQ.data.body ?? "");
    }
  }, [mineQ.data]);
  ...
  const save = useMutation({
    mutationFn: () =>
      saveFn({
        data: { target_type: targetType, target_id: targetId, rating, title: title.trim() || null, body: body.trim() || null },
      }),
    onSuccess: () => {
  ...
  const del = useMutation({
    mutationFn: () => delFn({ data: { target_type: targetType, target_id: targetId } }),
    onSuccess: () => {
  ...
  const sum = sumQ.data ?? { count: 0, avg: 0 };
    ...
    <section className="rounded-2xl border bg-card p-5 shadow-card md:p-6">
      ...
      {user && (
        <div className="mt-4 rounded-xl border bg-secondary/30 p-4">
          {mineQ.isLoading ? (
            <div className="grid h-16 place-items-center text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" /></div>
          ) : (
            <>
              <div className="flex items-center justify-between gap-2">
                <div className="text-xs font-bold uppercase tracking-wide text-muted-foreground">
                  {mineQ.data ? "Your review" : "Leave a review"}
                </div>
                {mineQ.data && !editing && (
              ...
              {mineQ.data && !editing ? (
                <div className="mt-2">
                  <StarRow value={mineQ.data.rating} />
                  {mineQ.data.title && <div className="mt-1 text-sm font-bold">{mineQ.data.title}</div>}
                  {mineQ.data.body && <p className="mt-1 text-xs text-muted-foreground">{mineQ.data.body}</p>}
                </div>
              ) : (
                <form
                ...
                >
                  ...
                  <div className="flex gap-2">
                    <button disabled={save.isPending} className="rounded-lg bg-primary px-3 py-1.5 text-xs font-bold text-primary-foreground shadow-pop disabled:opacity-50">
                      {save.isPending ? "Saving…" : mineQ.data ? "Update" : "Post review"}
                    </button>
                    {editing && (
              ...
              <li key={rv.id} className="rounded-xl border bg-background p-4">
                <div className="flex items-center gap-2">
                  <div className="grid h-8 w-8 place-items-center rounded-full bg-primary text-[11px] font-bold text-primary-foreground">
                    {(rv.author_name || "U").slice(0, 1).toUpperCase()}
                  </div>
                  <div className="min-w-0">
                    <div className="text-sm font-bold">{rv.author_name}</div>
