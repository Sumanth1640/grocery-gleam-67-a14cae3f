...
const couponFields =
  "id,code,description,discount_type,discount_value,min_order,max_discount,usage_limit,used_count,valid_from,valid_until,is_active";
...
export async function listActiveCoupons(): Promise<Coupon[]> {
  ...
  return ((data ?? []) as Coupon[]).filter(isCouponUsableNow);
}
