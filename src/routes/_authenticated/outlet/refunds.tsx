...
export const Route = createFileRoute("/_authenticated/outlet/refunds")({
  ...
  component: VerifyRefundsPage,
});

function VerifyRefundsPage() {
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className="font-display text-xl font-bold flex items-center gap-2"><ShieldCheck className="h-5 w-5" /> Refund requests to verify</h2>
          <p className="text-xs text-muted-foreground">Verify customer proof, then admin will issue the refund.</p>
        </div>
      ...
      {q.isLoading ? (
        ...
        <div className="space-y-3">
          ...
          {rows.length === 0 && <div className="rounded-2xl border bg-card p-8 text-center text-sm text-muted-foreground">No refund requests.</div>}
        </div>
      )}
    </div>
  );
...
function ProofThumbs({ urls }: { urls: string[] }) {
  ...
  if (urls.length === 0) return <div className="mt-2 text-xs text-muted-foreground italic">No proof uploaded by customer.</div>;
  return (
    <div className="mt-2">
...
function Card({ r, onAct }: { r: any; onAct: (s: "verified" | "rejected", note: string) => void }) {
  ...
  return (
    <div className="rounded-2xl border bg-card p-4 shadow-card">
      ...
      {r.verifier_note && <div className="mt-2 rounded-lg bg-muted/50 p-2 text-xs">Your note: {r.verifier_note}</div>}
