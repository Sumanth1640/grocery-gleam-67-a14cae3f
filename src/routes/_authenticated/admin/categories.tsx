...
type CatRow = { id: string; slug: string; name: string; image: string; tint: string; sort_order: number };
type Form = { id?: string; slug: string; name: string; image: string; tint: string; sort_order: string };
...
function CategoriesAdmin() {
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => { toast.success("Category saved"); qc.invalidateQueries({ queryKey: ["admin", "categories"] }); setForm(null); },
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <button onClick={() => setForm({ ...empty })} className="inline-flex items-center gap-1.5 rounded-xl bg-primary px-4 py-2 text-xs font-bold text-primary-foreground shadow-pop">
      ...
      {authLoading || cats.isLoading ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {(cats.data as CatRow[] | undefined ?? []).map((c) => (
            <div key={c.id} className="flex items-center gap-3 rounded-2xl border bg-card p-4 shadow-card">
              <div className="grid h-14 w-14 shrink-0 place-items-center overflow-hidden rounded-xl" style={{ background: c.tint }}>
              ...
              <div className="flex gap-1">
                <button
                  onClick={() => setForm({ id: c.id, slug: c.slug, name: c.name, image: c.image, tint: c.tint, sort_order: String(c.sort_order) })}
      ...
      {form && (
        <div className="fixed inset-0 z-50 grid place-items-end bg-black/40 sm:place-items-center" onClick={() => setForm(null)}>
          <div onClick={(e) => e.stopPropagation()} className="max-h-[90vh] w-full overflow-auto rounded-t-2xl bg-background p-6 shadow-pop sm:max-w-md sm:rounded-2xl">
