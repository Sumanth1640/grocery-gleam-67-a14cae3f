...
function ProductsAdmin() {
  ...
  const visible = ((products.data as ProductRow[] | undefined) ?? []).filter(
    (p) =>
      !q.trim() ||
              ...
              {visible.map((p) => (
                <tr key={p.id} className="border-b last:border-0 hover:bg-secondary/30">
                  ...
                  <td className="p-3 text-center">
                    <span
                      className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${p.in_stock ? "bg-success/15 text-success" : "bg-destructive/10 text-destructive"}`}
                    >
  ...
  return (
    ...
    >
      <div
        ...
        className="max-h-[90vh] w-full overflow-auto rounded-t-2xl bg-background p-6 shadow-pop sm:max-w-lg sm:rounded-2xl"
      >
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <Field label="ETA">
            <input className={cls} value={form.eta} onChange={(e) => set("eta", e.target.value)} />
          </Field>
          <Field label="Price (₹)">
            <input
              inputMode="numeric"
