              ...
              <div className="mt-5 rounded-2xl border bg-card p-5 shadow-card">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Your delivery partner</div>
                      <div className="font-display text-lg font-extrabold leading-tight">{(riderQ.data.riders as any).name}</div>
                      <div className="text-xs text-muted-foreground capitalize">
                        {(riderQ.data.riders as any).vehicle} {(riderQ.data.riders as any).vehicle_no || ""} ·{" "}
                        {riderQ.data.status === "picked_up" ? "On the way" : riderQ.data.status === "delivered" ? "Delivered" : "Assigned"}
                      </div>
                    </div>
                  </div>
                  {(riderQ.data.riders as any).phone && order.status !== "delivered" && (
                    <a
                      href={`tel:${(riderQ.data.riders as any).phone}`}
                      className="inline-flex items-center gap-1.5 rounded-xl bg-emerald-600 px-3 py-2 text-xs font-bold text-white shadow-pop hover:opacity-95"
                    >
