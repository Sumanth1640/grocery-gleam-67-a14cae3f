...
import { Clock, Leaf, ShieldCheck, Truck, Utensils, ArrowRight, Star, Trees } from "lucide-react";
import { furnitureItems as fallbackFurniture, type FurnitureItem } from "@/lib/furniture-data";
...
function FurniturePromo() {
  ...
  return (
    <section className="mx-auto max-w-7xl space-y-8 px-4 py-12">
      {/* Hero ad strips — fully dynamic, admin-managed */}
      {promos.map((p) => {
            ...
            <div>
              {p.eyebrow && (
                <div className="inline-flex items-center gap-1.5 rounded-full border border-foreground/15 bg-background/60 px-3 py-1 text-[11px] font-bold uppercase tracking-wider backdrop-blur">
                  <Trees className="h-3.5 w-3.5" /> {p.eyebrow}
                </div>
              )}
              <h3 className="mt-3 font-display text-3xl font-extrabold leading-tight tracking-tight md:text-5xl">
                {p.title}
                {p.highlight && (<><br /><span className="text-primary">{p.highlight}</span></>)}
              </h3>
              {p.blurb && <p className="mt-3 max-w-md text-sm text-foreground/75 md:text-base">{p.blurb}</p>}
              <span className="mt-5 inline-flex items-center gap-1.5 rounded-xl bg-foreground px-5 py-3 text-xs font-bold text-background shadow-pop">
                {p.cta_label} <ArrowRight className="h-3.5 w-3.5 transition group-hover:translate-x-0.5" />
              </span>
            </div>
              ...
              return (
                ...
                >
                  <div className="relative aspect-[4/5] overflow-hidden">
                    ...
                    {off > 0 && (
                      <div className="absolute right-3 top-3 rounded-full bg-discount px-2 py-1 text-[10px] font-extrabold text-white shadow-pop">{off}% OFF</div>
                    )}
